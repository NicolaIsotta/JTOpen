...
/**
 * The AS400ObjectFactory is used by a JNDI service provider to reconstruct
 * an object when it is retrieved from JNDI.
 *
 * <p>
 * For Example:
 * <blockquote><pre>
  ...
  try {
      ...
      AS400 as400 = pool.getConnection("localhost", "*CURRENT", "*CURRENT".toCharArray());
  } catch (Exception ex) {
      ex.printStackTrace();
...
public class AS400ObjectFactory implements ObjectFactory {
    ...
    public Object getObjectInstance(Object referenceObject,
        ...
        if (reference.getClassName().equals("com.ibm.as400.access.AS400ConnectionPool")) {
            AS400ConnectionPool pool = new AS400ConnectionPool(reference);
            return pool;
        } else {
